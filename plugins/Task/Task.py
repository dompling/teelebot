...
def Task(bot, message):
    ...
    if text[: len(prefix)] == prefix and count == 0:
        msg = (
            ...
            + "\n"
            + "<b>/taskfind</b> 查找任务 格式：指令+空格+标识"
            + "\n"
            + "<b>/taskclear</b> 移除所有任务"
            + "\n"
            + "<b>/taskstatus</b> 查看队列信息"
            + "\n\n"
            ...
            for task in taskList:
                ...
                ok, uid = bot.schedule.add(
                    ...
                    (
                        bot,
                        message["chat"]["id"],
                        task_content["commond"],
                        task_content["commond"],
                        message,
                        "HTML",
                    ),
                )
                task_content["uid"] = uid
        ...
        if len(text.split(" ")) >= 3:
            ...
            msg_content = str(text.split(" ", 2)[2])
            msg = (
                msg_content
                + "\n\n"
                + "<code>此消息为定时发送，周期"
            ...
            content = {"gap_key": _time, "commond": msg_content}
            taskList = db.select(user_id, "task")
            has_task = False
            if taskList:
                for task in taskList:
                    task_content = json.loads(task["content"])
                    uid = task_content["uid"]
                    has_task = task_content["commond"] == msg_content

            if not has_task:
                ok, uid = bot.schedule.add(
                    gap,
                    event,
                    (bot, message["chat"]["id"], msg_content, msg, message, "HTML"),
                )
                ...
                if ok:
                    ...
                    db.insert(user_id, json.dumps(content), "task")
                else:
                    msg = ""
                    if uid == "Full":
                        msg = "<b>队列已满</b>"
        ...
        if ok:
            ...
            if taskList:
                for task in taskList:
                    task_content = json.loads(task["content"])
                    uid = task_content["uid"]
                    msg += f"<b>{task_content['commond']}</b>：{uid} {task_content['gap_key']}\n"
        else:
